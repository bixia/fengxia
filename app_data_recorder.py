...
from trader_event import EVENT_TICK, EVENT_CONTRACT
from trader_object import ContractData, TickData, SubscribeRequest, BarData
...
class RecorderEngine(BaseEngine):
    ...
    def load_setting(self):
        ...
        self.tick_recording = setting.get("tick", {})
        self.bar_recordings = setting.get("bar", {})

    def save_setting(self):
        setting = {
            "tick": self.tick_recording,
    ...
    def add_tick_recording(self, vt_symbol: str):
        if vt_symbol in self.tick_recording:
    ...
    def remove_tick_recording(self, vt_symbol: str):
        if vt_symbol not in self.tick_recording:
            self.write_log(f"不在Tick记录列表中：{vt_symbol}")
            return
        self.tick_recording.pop(vt_symbol)
        self.save_setting()
        self.put_event()
    ...
    def update_tick(self, tick: TickData):
        if tick.vt_symbol in self.tick_recording:
    ...
    def process_contract_event(self, event: Event):
        ...
        if vt_symbol in self.tick_recording or vt_symbol in self.bar_recordings:
            self.subscribe(contract)
    ...
    def put_event(self):
        tick_symbols = list(self.tick_recording.keys())
        tick_symbols.sort()
